...
def load_master_data():
    ...
    try:
        if os.path.exists("live_pme_updates.csv"):
            ...
            overrides['Pl.No.'] = pd.to_numeric(overrides['Pl.No.'], errors='coerce')
            overrides['Date of test'] = pd.to_datetime(overrides['Date of test'], format='%d-%m-%Y', errors='coerce')
            raw_latest = pd.concat([raw_latest, overrides], ignore_index=True)
    except Exception:
        pass
...
if page == "Enterprise Overview":
    ...
    with tab_latest:
        ...
        if not df_pme_latest.empty:
            ...
            if not overdue_pme.empty:
                if st.session_state.get('username') == 'admin':
                    st.info("✏️ **Admin Mode:** Double-click the 'Date of test' cell below to update records (Format: DD-MM-YYYY).")
                        ...
                        if st.button("💾 Save Dates", type="primary"):
                            ...
                            if not changed.empty:
                                ...
                                if os.path.exists("live_pme_updates.csv"):
                                    ...
                                    updated.to_csv("live_pme_updates.csv", index=False)
                                else:
                                    update_df.to_csv("live_pme_updates.csv", index=False)
                                    
                                st.cache_data.clear() 
                                st.success("✅ Records updated! Refreshing...")
                                st.rerun() 
                            else:
    ...
    with tab_monthly:
        ...
        if not df_pme_latest.empty:
            ...
            if not df_monthly.empty:
                ...
                with col_table: st.dataframe(monthly_counts[['Month', 'Total Employees Examined']].reset_index(drop=True), use_container_width=True)
                with col_chart:
    ...
    with tab_monthly:
        ...
        if not df_refresher.empty and 'Refresher Last Date' in df_refresher.columns:
            ...
            if not df_monthly.empty:
                ...
                monthly_counts = monthly_counts.sort_values('Sort Key', ascending=False)
                col_table, col_chart = st.columns([1, 2])
                with col_table: st.dataframe(monthly_counts[['Month', 'Total Trained']].reset_index(drop=True), use_container_width=True)
                with col_chart:
    ...
    with tab_charts:
        ...
        if not df_firstaid.empty and 'First Aid Last Year' in df_firstaid.columns:
            ...
            with col_fa1:
                ...
                fig_fa_bar = px.bar(fa_year_summary, x='First Aid Last Year', y='Trained Personnel', text_auto=True, title="Annual First Aid Certifications", color_discrete_sequence=['#2ca02c'])
            ...
            with col_fa2:
                if 'First Aid Type' in df_firstaid.columns:
                    ...
                    fig_fa_pie = px.pie(fa_type_summary, values='Total Trained', names='First Aid Type', hole=0.4, title="Breakdown by First Aid Type", color_discrete_sequence=px.colors.qualitative.Set2)
    ...
    if st.session_state.get('username') != 'admin':
        ...
        with tab_upload:
            ...
            target_file = st.selectbox("Select File to Replace:", ["PME 2026.xlsx", "PME 2025.xlsx", "Training_Desk_Final_OEM_Service_v2 (2).xlsx"])
            ...
            if uploaded_file is not None:
                if st.button("🔄 Overwrite File on Server"):
                    with open(target_file, "wb") as f: f.write(uploaded_file.getbuffer())
                    st.cache_data.clear()
